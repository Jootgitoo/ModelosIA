...
def get_flight_status(flight_id):
    '''
    Busca el estado del vuelo en la base de datos
    '''
    print(f"DEBUG: Buscando estado del vuelo {flight_id}")
    return flight_status_db.get(flight_id, "Estado desconocido para este vuelo")
    ...
    "function": {
        ...
        "parameters": {
            ...
            "properties": {
                "flight_id": {
                    "type": "string",
                    "description": "The flight ID to check",
                },
            },
            "required": ["flight_id"],
        },
...
def chat_logic(history):
    ...
    payload = {
        "model": OLLAMA_MODEL,
        "messages": messages,
        "tools": [tool_definition, status_function], # Le decimos a Ollama qué funciones puede usar
        "stream": False,
    ...
    try:
        ...
        for tool in tool_calls:
            ...
            if function_name == "get_flight_status":
                ...
                #Ejecutamos nuestra función Python local
                status_result = get_flight_status(flight_id)
                
                #Preparamos el mensaje de resultado para el modelo
                tool_msg = {
                    "role": "tool",
                    "content": json.dumps({"status": status_result}),
                    "name": function_name
                }
                #Id de llamada (requerido por estándares modernos)
                if "id" in tool: tool_msg["tool_call_id"] = tool["id"]
