...
def generar_respuesta(prompt, modelo_seleccionado):
    # Manejo de error: pregunta vacía
    if not prompt.strip():
        return "⚠️ Por favor, escribe una pregunta antes de continuar."

    # Llamada a la función correspondiente según el modelo seleccionado
    if modelo_seleccionado == "ChatGPT":
        return respuesta_chatgpt(prompt)

    elif modelo_seleccionado == "Gemini":
        return respuesta_gemini(prompt)

    elif modelo_seleccionado == "Claude":
        return respuesta_claude(prompt)

    # Manejo de error: modelo no válido
    else:
        return "❌ El modelo seleccionado no es válido. Por favor, elige uno correcto."


def respuesta_chatgpt(prompt):
    return f"### ChatGPT\n\nHas preguntado:\n> {prompt}"

def respuesta_gemini(prompt):
    return f"### Gemini\n\nHas preguntado:\n> {prompt}"

def respuesta_claude(prompt):
    return f"### Claude\n\nHas preguntado:\n> {prompt}"
